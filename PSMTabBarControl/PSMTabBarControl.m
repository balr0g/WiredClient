...
- (id)initWithFrame:(NSRect)frame
{
    ...
    if (self) {
		...
		// Axel Andersson: move from -awakefromNib
		// resize
...
- (void)setStyleNamed:(NSString *)name
{
	...
	else if ([name isEqualToString:@"Wired"]){
		style = [[PSMWiredTabStyle alloc] init];
	}
	else {
        style = [[PSMMetalTabStyle alloc] init];
    }
...
- (void)addTabViewItem:(NSTabViewItem *)item
{
    ...
    [cell release];
    if([_cells count] == (unsigned int) [tabView numberOfTabViewItems]){
...
- (void)update
{
    ...
    if ((unsigned int) [tabView numberOfTabViewItems] != [_cells count]) {
        return;
    }
